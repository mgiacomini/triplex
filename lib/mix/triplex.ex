defmodule Mix.Triplex do
  ...
  def migrations_path(repo),
    do: repo |> source_repo_priv() |> Path.join("tenant_migrations")
  ...
  defp relative_migrations_path(repo, config),
    do: repo |> migrations_path() |> Path.relative_to(Project.app_path(config))
end
